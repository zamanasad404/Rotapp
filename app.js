...
function save(){ if(!state.workspace) return;
...
// Deep-link: role from hash (#role=Supervisor)
function parseHashRole(){
  const m = location.hash.match(/role=([^&]+)/i);
...
$("#signInBtn").addEventListener("click", () => {
  ...
  // Init role filter
  const deepRole = parseHashRole();
  state.roleFilter = deepRole && (state.roles.includes(deepRole) || deepRole==="Unassigned (open)") ? deepRole : 'All roles';
  save();
...
function renderRoles(){
  ...
  // Role filter populate
  const rf = $("#roleFilter");
  rf.innerHTML = "";
  ["All roles","Unassigned (open)", ...state.roles].forEach(r => {
    const opt = document.createElement("option"); opt.value = r; opt.textContent = r;
    if(r===state.roleFilter) opt.selected = true;
    rf.appendChild(opt);
  });
}
$("#roleFilter").addEventListener("change", (e) => {
  state.roleFilter = e.target.value;
  // Update hash for shareable role rota
  const r = state.roleFilter;
  if(r==="All roles") history.replaceState(null, "", location.pathname);
  else location.hash = `role=${encodeURIComponent(r)}`;
  renderRota();
});
...
function applyRoleFilter(items){
  if(state.roleFilter==="All roles") return items;
  return items.filter(it => (state.roleFilter==="Unassigned (open)" ? it.staffName==="Unassigned (open)" : it.role===state.roleFilter));
}

function renderRota(){
  renderWeekLabel();
  const {start} = currentWeekRange(state.weekOffset);
  // Merge assignments + pseudo "Unassigned" for open shifts
  ...
  const combinedBase = state.assignments.concat(pseudo);
  const combined = applyRoleFilter(combinedBase);
  ...
  Object.keys(byStaff).forEach(name => {
    ...
    days.forEach(day=>{
      const items = (byStaff[name][day.key]||[]).map(it => `${it.start}-${it.end} <span class="badge">${escapeHtml(it.role)}</span>${it.location?` <span class="meta">@ ${escapeHtml(it.location)}</span>`:""}`).join("<br>");
      html += `<td>${items||""}</td>`;
    });
    html += `</tr>`;
  });
  ...
  // GANTT — fix day index + role colours
  const gwrap = $("#rotaGanttView"); let ghtml = `<div class="gantt">`;
  Object.keys(byStaff).forEach(name => {
    ghtml += `<div class="row"><div><b>${escapeHtml(name)}</b></div><div class="bar-wrap">`;
    const items = Object.values(byStaff[name]).flat();
    items.forEach(it => {
      const dayIdx = Math.max(0, Math.min(6, Math.floor((new Date(it.date) - new Date(start)) / 86400000)));
      const x0 = toMinutes(it.start), x1 = toMinutes(it.end);
      const leftPct = ((dayIdx + x0/1440)/7)*100;
      const widthPct = Math.max(1.5, ((x1-x0)/1440)/7*100);
      const title = `${fmtDate(it.date)} ${it.start}-${it.end} ${it.role}${it.location?` @ ${it.location}`:""}`;
      const color = roleColor(it.role);
      ghtml += `<div class="bar" title="${escapeHtml(title)}" style="left:${leftPct}%; width:${widthPct}%; background:${color}"></div>`;
    });
    ghtml += `</div></div>`;
  });
...
// Share role rota link
$("#shareRoleLinkBtn").addEventListener("click", () => {
  const r = state.roleFilter;
  const url = new URL(location.href);
  if(r==="All roles") url.hash = "";
  else url.hash = `role=${encodeURIComponent(r)}`;
  navigator.clipboard?.writeText(url.toString());
  alert("Role rota link copied.");
});

// Export / Share — use dark background for canvas so text is readable
let exportTarget = $("#rotaTableView");
function exportCanvasTarget(){ return exportTarget; }
...
$("#exportPdfBtn").addEventListener("click", async () => { await exportAsPdf(exportCanvasTarget()); });
$("#exportPngBtn").addEventListener("click", async () => { await exportAsPng(exportCanvasTarget()); });
$("#shareImageBtn").addEventListener("click", async () => { await shareImage(exportCanvasTarget()); });

async function exportAsPdf(node){
  if(!node){ alert("Nothing to export"); return; }
  try{
    const canvas = await html2canvas(node, {backgroundColor:"#12131a", scale:2});
...
function buildTextExport(){
  const {start} = currentWeekRange(state.weekOffset);
  const pseudo = state.openShifts.map(sh => ({ id: sh.id, staffName:"Unassigned (open)", date: sh.date, start: sh.start, end: sh.end, role: sh.role, location: sh.location, notes: sh.notes }));
  const combined = applyRoleFilter(state.assignments.concat(pseudo));
  const byStaff = groupAssignmentsByStaffInRange(combined, start, addDays(start,6));
  let lines = [`Rota ${fmtDate(start)} – ${fmtDate(addDays(start,6))}${state.roleFilter!=='All roles' ? ` [${state.roleFilter}]` : ''}`];
  Object.keys(byStaff).forEach(name => {
    lines.push(`\n${name}:`);
...
function roleColor(role){
  let hash = 0; for(let i=0;i<role.length;i++){ hash = role.charCodeAt(i) + ((hash<<5)-hash); }
  const hue = Math.abs(hash)%360; const sat = 65; const light = 50;
